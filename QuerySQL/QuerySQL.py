...
import numpy as np
#BETTER WAY TO GO ABOUT THIS GOING FORWARD IS TO HAVE MODULARITY. CREATE MODULES,  

#---------------------------------------------------------------------------------------
#VALIDATE ALL THE INPUTS

#pull each awd number and check if the awd complies.
...
def queryDB(values):
    ...
    dataframes ={}    
    if(len(rate_codes) >= 2 and len(awds) >= 2):
        for i in rate_codes:
            for j in awds:
    ...
    default_query= query_statement(default_segment_description, default_car_groups,default_charge_descriptions, default_region, default_start_date, default_end_date, default_awd_num, default_rate_codes )
...
while True:
    ...
    if event == sg.WIN_CLOSED :
        ...
        if( isinstance(df, dict) ):
            ...
            with pd.ExcelWriter(download_path(values['-PATH-']), mode = 'w', engine = 'openpyxl') as writer:
                for i  in df.keys():
